...
int run_run_change(std::vector<double> u0, std::vector<double> tmp, double h, int p, double *norma) //u0 - начальное значение; tmp - значение в yn, p - порядок
{
	...
	*norma = -log(norm((u0) / (pow(2, p) - 1), (tmp) / (pow(2, p) - 1)));
	if (norm((u0) / (pow(2, p) - 1), (tmp) / (pow(2, p) - 1)) < EPS)
		return 0;		//шаг подходит
	return 1;		//шаг не подходит
}
...
std::vector<double> run_k_2_2_change(std::vector<double> u0, double T, double h)
{
	...
	for (double i = 0; i <= T; i += h)
	{
		...
		{
			h *= min(facmax, max(facmin, fac * pow(EPS*0.1 / EPS, 2)));
			u0 = tmp1;
			fout << norma << std::endl;
			continue;
		}
...
std::vector<double> run_k_4_4(std::vector<double> u0, double T, double h, int flag, char nbr, int step)
{
	std::vector<double> k_n1(dim);
	std::vector<double> k_n2(dim);
...
std::vector<double> run_k_4_4_change(std::vector<double> u0, double T, double h)
{
	...
	for (double i = 0; i <= T; i += h)
	{
		...
		{
			h *= min(facmax, max(facmin, fac * pow(EPS*0.1 / EPS, 4)));
			u0 = tmp1;
			fout << norma << std::endl;
			continue;
		}
